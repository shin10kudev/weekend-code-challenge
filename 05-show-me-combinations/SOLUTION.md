...
This is a little tricky, but basically if we look at this array in reverse from the second to the last value, `[4, 3, 2, 1]`, we can use this as a reference kind of like the keys in a hash for generating our pairs. We know for istance that `1` will be matched with `4` numbers, `2` with `3` numbers, `3` with `2`, numbers and so on.
